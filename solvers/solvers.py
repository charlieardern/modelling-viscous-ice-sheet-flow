...
def numerical_fv_implicit_general_b(h_0, x, b, num_microsteps, num_steps, t_final, x_G_0, g, nu, rho_w, rho, a, L, test_mode=False, hide_output=False, sheet_accumulation=True):
    ...
    def x_G_dot(h, x_G, h_G, b_G, db_G_dchi):
        dh_dchi = (8*h_G+h[-2]-9*h[-1])*N/3
        term_1 = 1.5*(rho_w*b_G/rho)**2*(1/x_G)*(dh_dchi**2-x_G**2)/((rho_w-rho)*db_G_dchi/rho-dh_dchi)
        term_2 = -(rho_w*b_G/rho)**2*dh_dchi/x_G
        return np.min([term_1, term_2])
    
    def advective_term(h, x_G, h_G, dx_G_dt):
    ...
    # The inputs for these are already taken to be dimensionless:
    #b = b
    #x = x
    h = h_0
    x_G = x_G_0

    b_G = np.interp(np.array(x_G), x, b)
    db_G_dchi = np.interp(np.array(x_G), x, np.gradient(b, x))
    h_G = (rho_w-rho)*b_G/rho
    H_G = h_G + b_G
    ...
    x_G_list.append(x_G)
    x_G_full_list.append(x_G)
    x_G_dot_list.append(x_G_dot(h, x_G, h_G, b_G, db_G_dchi))
    t_G_list.append(0)
    H_G_list.append(H_G)
    q_G_list.append(-g/(3*nu)*H_G**3*((8*h_G+h[-2]-9*h[-1])*N/(3*x_G)))
    ...
    for i in tqdm(range(num_steps), disable=hide_output):
        ...
        for ii in range(num_microsteps):
            ...
            b_i_minus = np.interp(chi_minus*x_G, x, b)
            b_i_plus = np.interp(chi_plus*x_G, x, b)
            

            dx_G_dt = x_G_dot(h, x_G, h_G, b_G, db_G_dchi)
            k_plus = K_plus(h, b_i_plus)
            k_minus = K_minus(h, b_i_minus)
            C_n = delta_t*N**2/(x_G**2)
            B_n = (rho_w*b_G/rho)**3/3
