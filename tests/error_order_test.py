...
microstep_list = [1, 5, 10, 20, 40, 80, 160, 320, 640, 1280]
rmse_list = []
N = 200
...
num_microsteps = 50000
# _, h_fine, _, _ = numerical_fv_implicit(h_0, 10000, num_steps, t_final, x_G_0, g, nu, rho_w, rho, alpha, a, L, test_mode=True)
# h_fine = h_fine[:,-1]/(B*C)
rmse_list = []

h_0 = 1-np.linspace(0,1,num=1000)+0.002+0.1
x, h_fine, _, _ = numerical_fv_implicit(h_0, num_microsteps, num_steps, t_final, x_G_0, g, nu, rho_w, rho, alpha, a, L, test_mode=True, sheet_accumulation=False)
h_fine = h_fine[:,-1]/(B*C)
...
plt.figure(figsize=(8,5), dpi=300)
plt.plot(1/np.array(N_list), rmse_list)
plt.title("RMSE for steady state", fontname="Latin Modern Roman", fontsize=16)
