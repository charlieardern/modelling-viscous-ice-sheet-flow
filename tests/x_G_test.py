...
fig, ax = plt.subplots(constrained_layout=True, figsize=(7,6), dpi=300)
ax.set_xlabel(r"$\hat t$ (dimensionless)", fontname="Latin Modern Roman", fontsize=14)
ax.set_ylabel(r"$\hat x$ (dimensionless)", fontname="Latin Modern Roman", fontsize=14)
...
ax.set_ylim(np.min(x_G), np.max(x_G)+0.2*(np.max(x_G)-np.min(x_G)))
ax.set_xlim(0, np.max(timesteps))
x_G_steady = 1/(A+eps)
ax.plot([timesteps[0], timesteps[-1]], [x_G_steady, x_G_steady], alpha=0.2, c="blue", linestyle="--")
