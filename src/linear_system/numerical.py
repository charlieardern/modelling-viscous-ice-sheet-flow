...
def compute_numerical_explicit(h_0, num_microsteps, num_t_steps, t_final, dim, L, h_G, a, k):
...
def compute_numerical_implicit(h_0, num_microsteps, num_t_steps, t_final, dim, L, h_G, a, k):
    """Computes numerical solution for initial problem with h_0(x)=-h_G*x/L + 2*h_G with implicit time steps.
        ...
        Returns h which is a numpy array of shape (dim, num_t_steps, 1) representing the solution on the specified domain in space and time.
    """
    print("Calculating implicit numerical solution...")
    w = L/dim
    ...
    M = np.eye(dim)-delta_t*A
    M_inv = np.linalg.inv(M)

    for i in range(num_t_steps-1):
        for ii in range(num_microsteps):
            h_i = M_inv@(h_i+delta_t*B)
        h_list.append(h_i)
